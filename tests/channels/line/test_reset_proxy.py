...
from channels.line.proxy import main
...
@pytest.fixture
def mock_fs_tools():
    with patch('channels.line.proxy.Path.exists') as m_exists, \
...
def test_reset_proxy_full_cleanup(mock_psutil_tools, mock_fs_tools):
    ...
    with patch('channels.line.proxy.argparse.ArgumentParser.parse_args') as m_args:
        args = MagicMock()
        args.chat = "Test Chat!"
        args.clear_log = True
        ...
        m_remove.assert_any_call(Path.home() / ".chat-agent" / "locks" / "Test_Chat_.pid")
        m_remove.assert_any_call(Path.home() / ".chat-agent" / "logs" / "Test Chat!.log")
