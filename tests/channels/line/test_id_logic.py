...
@pytest.mark.asyncio
async def test_find_chats_returns_chat_id():
    """Verify that find_chats returns chat_id and deduplicates by it."""
    mock_page = MagicMock()
...
async def test_select_chat_passes_chat_id():
    ...
    with patch("channels.line.driver.is_logged_in", return_value=True), \
         ...
         patch("channels.line.driver.open_chat", return_value={"status": "success"}) as mock_open:
