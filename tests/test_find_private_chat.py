...
@pytest.mark.asyncio
async def test_find_private_chat_success():
    ...
    with patch("playwright.async_api.async_playwright") as mock_p:
        ...
        with patch("line_utils.get_line_page", return_value=mock_page), \
             patch("line_utils.find_private_chat", return_value=mock_res):
            
            import mcp_server
            result = await mcp_server.find_private_chat(chat_name="楊Kurt", port=9222)
            data = json.loads(result)
            assert data["status"] == "success"
            assert data["chat_name"] == "楊Kurt"

@pytest.mark.asyncio
async def test_find_private_chat_not_found():
    ...
    with patch("playwright.async_api.async_playwright") as mock_p:
        ...
        with patch("line_utils.get_line_page", return_value=mock_page), \
            ...
            import mcp_server
            result = await mcp_server.find_private_chat(chat_name="Unknown", port=9222)
            data = json.loads(result)
