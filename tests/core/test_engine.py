...
async def test_engine_runtime_timeout_logging():
    ...
    with patch("channels.line.driver.select_chat", new_callable=AsyncMock, return_value={"status": "success"}), \
         patch("channels.line.driver.extract_messages", new_callable=AsyncMock, return_value=[{"text": "m", "sender": "Chat"}]), \
         patch("core.history.HistoryManager.write_log") as mock_log, \
...
async def test_run_engine_cli_reports_timeout(capsys):
    ...
    with patch("channels.line.run_engine.LineProxyEngine") as mock_engine_class, \
